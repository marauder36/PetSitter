...
class CompleteRegistrationActivity : AppCompatActivity() {
    ...
    private lateinit var currNume:    EditText
    private lateinit var currPrenume: EditText
    private lateinit var currUsername:EditText
    private lateinit var currTelefon: EditText
    ...
    override fun onStart() {
        ...
        if(firstAuth.currentUser!=null){
            ...
            startActivity(Intent(this@CompleteRegistrationActivity, SelectorActivity::class.java))
            finish()
        }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        currentUser.addListenerForSingleValueEvent(object : ValueEventListener{
            ...
            override fun onCancelled(error: DatabaseError) {
               // Toast.makeText(this@CompleteRegistrationActivity,"Couldn't retrieve data: ${error.toException()}",Toast.LENGTH_LONG).show()
    ...
    private fun saveDataToRTDB(currentUser: DatabaseReference) {
        ...
        database.getReference(prefType).get()
            .addOnSuccessListener {snapshot->
                ...
                if (cnt!=0){
                    currUsername.setError("Username-ul este deja folosit")
                    currUsername.requestFocus()
                }
                else{
                    currentUser.addListenerForSingleValueEvent(object : ValueEventListener{
                        override fun onDataChange(snapshot: DataSnapshot) {
                            ...
                            if (currNume.text.isEmpty()) {
                                currNume.setError("Numele nu poate fi gol !")
                                currNume.requestFocus()
                                nothingEmpty++
                            }
                            else if (currPrenume.text.isEmpty()) {
                                currPrenume.setError("Prenumele nu poate fi gol !")
                                currPrenume.requestFocus()
                                nothingEmpty++
                            }
                            else if (currUsername.text.isEmpty()) {
                                currUsername.setError("Username-ul nu poate fi gol !")
                                currUsername.requestFocus()
                                nothingEmpty++
                            }
                            if (currTelefon.text.isEmpty()) {
                                currTelefon.setError("Te rog introdu un numar de telefon !")
                                currTelefon.requestFocus()
                                nothingEmpty++
                            } else if (currTelefon.text.toString().toCharArray().size != 10) {
                                currTelefon.setError("Numarul de telefon trebuie sa contina 10 numere !")
                                currTelefon.requestFocus()
                                nothingEmpty++
                            }

                            else if (nothingEmpty == 0)
                            {
                                ...
                                databaseReference.child(mAuth1.currentUser?.uid.toString()).child("hasCompletedRegi").setValue("yes")
    ...
    private fun didDataChange(callback: (Boolean?)->Unit){
        ...
        currentUser.addListenerForSingleValueEvent(object :ValueEventListener{
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                didDataChange= (numeCurent!=numeRTDB || prenumeCurent!=prenumeRTDB || usernameCurent!=usernameRTDB
                        ||telefonCurent!=telefonRTDB )
