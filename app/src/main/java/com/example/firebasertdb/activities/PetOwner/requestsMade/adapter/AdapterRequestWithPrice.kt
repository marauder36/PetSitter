package com.example.firebasertdb.activities.PetOwner.requestsMade.adapter
...
class AdapterRequestWithPrice(private var dataList:MutableList<ServiceRequestClassPrice>,private val context: Context):RecyclerView.Adapter<AdapterRequestWithPrice.ViewHolderClass>() {
    ...
    fun setData(newDataList: MutableList<ServiceRequestClassPrice>) {
        dataList = newDataList
    }

    fun getClickedItem(position: Int): ServiceRequestClassPrice?{
    ...
    override fun onBindViewHolder(holder: ViewHolderClass, position: Int) {
        ...
        holder.rasaPetRequest.text="Pret serviciu: ${currentItem.pretServiciuCerut} RON"
        holder.statusRequest.text= currentItem.status

        if (holder.statusRequest.text.toString()=="Accepted" )
            holder.statusRequest.setTextColor(Color.BLUE)

        else if (holder.statusRequest.text.toString()=="Pending" ) {
            val color = ContextCompat.getColor(context, R.color.yellow_pending)
            holder.statusRequest.setTextColor(color)
        }

        else if (holder.statusRequest.text.toString()=="Payed" || holder.statusRequest.text.toString()=="Completed")
