...
class PetSelectActivity : AppCompatActivity() {
    ...
    override fun onStart() {
        ...
        if (firstAuth.currentUser != null) {
            ...
            startActivity(Intent(this@PetSelectActivity, SelectorActivity::class.java))
            finish()
        }
    ...
    private fun initRTDB(){
        ...
        databaseReference = database.getReference("Petowner")
    ...
    private fun initRecycler() {
        ...
        adapter.setOnItemClickListener { position ->
            ...
            returnIntent.putExtra(Constants.pet_ales_pet_owner,petsID[position])
    ...
    private fun showDialogForAction(position: Int) {
        val dialog = Dialog(this@PetSelectActivity)
        ...
        if (position >= 0 && position < datalist.size ) {
            ...
            currentUserPets.child(childName).removeValue()
                .addOnSuccessListener {
                    ...
                    //Toast.makeText(this, "Deleted pet from database", Toast.LENGTH_LONG).show()
                }
                .addOnFailureListener {
                    //Toast.makeText(this, "Could not delete data from database", Toast.LENGTH_LONG).show()
                }
        } else {
            //Toast.makeText(this, "Could not delete, index out of bounds", Toast.LENGTH_LONG).show()
        }
    }
